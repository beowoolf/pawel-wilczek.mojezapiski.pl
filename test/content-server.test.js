...
describe('module', () => {
  ...
  test('$content() on root directory with only', async () => {
    ...
    expect(items).toEqual(
      expect.arrayContaining([{ title: 'Home' }, { title: 'About' }])
    )
  })
  ...
  test('$content() on file with without', async () => {
    const items = await $content('home')
      .without(['body', 'createdAt', 'updatedAt', 'toc'])
      .fetch()
  ...
  test('$content() on directory with surround of 404 slug', async () => {
    ...
    expect(items).toEqual([null, null])
  })

  test('$content() on directory with surround', async () => {
    const items = await $content('articles')
      .surround('understanding-how-fetch-works-in-nuxt-2-12')
      .sortBy('date', 'desc')
      .fetch()
  ...
  test('$content() on directory with surround as first', async () => {
    const items = await $content('articles')
      .surround('build-dev-to-clone-with-nuxt-new-fetch')
      .sortBy('date', 'desc')
      .fetch()
  ...
  test('$content() on directory with surround as last', async () => {
    const items = await $content('articles')
      .surround('introducing-smart-prefetching')
      .sortBy('date', 'desc')
      .fetch()
  ...
  test('$content() on directory with surround (path)', async () => {
    const items = await $content('positioned', { deep: true })
      .sortBy('position', 'asc')
      .surround('/positioned/category-1/challenge')
      .fetch()
  ...
  test('$content() on directory with surround as first (path)', async () => {
    const items = await $content('positioned', { deep: true })
      .sortBy('position', 'asc')
      .surround('/positioned/category-1/introduction')
      .fetch()
  ...
  test('$content() on directory with surround as last (slug)', async () => {
    const items = await $content('positioned', { deep: true })
      .sortBy('position', 'asc')
      .surround('/positioned/category-2/answer')
      .fetch()
  ...
  test('$content() on directory with surround duplicated slug (path)', async () => {
    const items = await $content('positioned', { deep: true })
      .sortBy('position', 'asc')
      .surround('/positioned/category-2/introduction')
      .fetch()
  ...
  test('$content() on subdirectory', async () => {
    const items = await $content('articles', '2020', '04')
      .only(['title'])
      .fetch()
  ...
  test('$content() on directory with deep', async () => {
    const items = await $content('articles', { deep: true })
      .only(['title'])
      .fetch()
  ...
  test('$content() on subdirectory with deep', async () => {
    const items = await $content('articles', '2020', { deep: true })
      .only(['title'])
      .fetch()
